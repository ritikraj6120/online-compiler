...
const computeCpp14 = (req, res) => {
	...
	let compileCommand = `g++ -std=c++14 ./uploads/${codefile} -o ./executable/${execFilename}`
	...
	// 	let { input } = req.files
	// 	let inputfile = input[0].filename
	// 	const cpp = spawn('timeout', ['5s', execCommand, '<', `./uploads/${inputfile}`]);
	// 	cpp.on('error', (err) => {
